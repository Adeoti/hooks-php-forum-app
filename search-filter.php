...
$key = "Zero Search";
if(isset($_GET['qkey'])){
	$key = htmlspecialchars($_GET['qkey']);
		if(!strlen($key)>0){
			$key = "Empty Search";
		}
}
...
<!DOCTYPE html>
	<html>
	<head>
		<title><?php echo "Search Pane for ".$key; ?></title>
		...
		<div class="wrapper">
			<?php include("header.php"); ?>
				...
				<!--To be altered by PHP-->
									...
									$pageNumber = "";
										//## Pagination Starts
											$rn = $con -> prepare("SELECT * FROM discussion WHERE title LIKE :search");
											$dt = "%".$key."%";
												$rn -> bindParam(':search',$dt);
												$pn = "";
												if($rn -> execute()){
													echo $row_num = $rn -> rowCount();
											...
											if($pn > 1 && $pn < $lastpage){
												...
												$pageNumber .= $centerControl;
													...
													echo "<script>window.location.href='search-filter.php?qkey=$key&pn=1'</script>";
												}
										...
										//## Pagination Ends
										$fetchPosts = $con -> prepare("SELECT * FROM discussion WHERE title LIKE :search ORDER BY ref DESC LIMIT $start,$rpp");
										$dt = "%".$key."%";
										$fetchPosts -> bindParam(':search',$dt);
										$fetchPosts -> execute();
